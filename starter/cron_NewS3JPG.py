...
"""
Cron job to check for new article S3 JPG and start workflows
"""

class cron_NewS3JPG(object):

  def start(self, ENV = "dev"):
    # Specify run environment settings
    settings = settingsLib.get_settings(ENV)
    
    ping_marker_id = "cron_NewS3JPG"
    ...
    # Check for S3 JPG files that were updated since the last run
    ...
    S3_item_list = db.elife_get_article_S3_file_items(file_data_type = "jpg", latest = True, last_updated_since = last_startDate)
    
    logger.info('JPG files updated since %s: %s' % (last_startDate, str(len(S3_item_list))))
    ...
    if(len(S3_item_list) <= 0):
      ...
      try:
        starter_name = "starter_PublishJPG"
