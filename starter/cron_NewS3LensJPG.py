...
"""
Cron job to check for new lens JPG zip files on S3 and start workflows
"""

class cron_NewS3LensJPG(object):

  def start(self, ENV = "dev"):
    # Specify run environment settings
    settings = settingsLib.get_settings(ENV)
    
    ping_marker_id = "cron_NewS3LensJPG"
    ...
    # 
    file_list = db.elife_get_lens_jpg_S3_file_items(last_updated_since = last_startDate)
    
    logger.info('Lens JPG zip files updated since %s: %s' % (last_startDate, str(len(file_list))))
    ...
    if(len(file_list) <= 0):
      ...
      try:
        starter_name = "starter_PublishLensJPG"
