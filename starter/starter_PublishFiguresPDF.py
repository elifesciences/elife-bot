...
"""
Amazon SWF PublishFiguresPDF starter
"""

class starter_PublishFiguresPDF():
	...
	def start(self, ENV = "dev", all = None, last_updated_since = None, docs = None, doi_id = None):
		...
		if(docs):
			for doc in docs:
				...
				workflow_id = "PublishFiguresPDF_%s" % (id_string)
				workflow_name = "PublishFiguresPDF"
				workflow_version = "1"
				child_policy = None
	...
	def get_docs_from_SimpleDB(self, ENV = "dev", last_updated_since = None, doi_id = None):
		...
		if(last_updated_since is not None):
			xml_item_list = db.elife_get_article_S3_file_items(file_data_type = "figures", latest = True, last_updated_since = last_updated_since)
		elif(doi_id is not None):
			xml_item_list = db.elife_get_article_S3_file_items(file_data_type = "figures", latest = True, doi_id = doi_id)
		else:
			# Get all
			xml_item_list = db.elife_get_article_S3_file_items(file_data_type = "figures", latest = True)
