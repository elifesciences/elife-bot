...
"""
Cron job to check for new article S3 PDF and start workflows
"""

class cron_NewS3PDF(object):

  def start(self, ENV = "dev"):
    # Specify run environment settings
    settings = settingsLib.get_settings(ENV)
    
    ping_marker_id = "cron_NewS3PDF"
    ...
    S3_item_list = db.elife_get_article_S3_file_items(file_data_type = "pdf", latest = True, last_updated_since = last_startDate)
    
    logger.info('PDF files updated since %s: %s' % (last_startDate, str(len(S3_item_list))))
    ...
    if(len(S3_item_list) <= 0):
      ...
      try:
        starter_name = "starter_PublishPDF"
