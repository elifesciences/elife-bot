...
def build_digest(input_file, temp_dir, logger=None, digest_config_object=None):
    ...
    try:
        ...
        if logger:
            logger.exception(
                "exception in EmailDigest build_digest: %s" % traceback.format_exc()
            )
        return False, None
    return True, digest
...
def digest_jats(digest_content, logger=None):
    ...
    try:
        ...
        if logger:
            logger.exception(
                "exception in digest_provider digest_jats: %s" % traceback.format_exc()
            )
...
def outbox_file_dest_resource(storage_provider, digest, bucket_name, file_path):
    ...
    dest_file_name = new_file_name(
        msid=utils.msid_from_doi(digest.doi), file_name=file_name
    )
...
def image_file_name_from_s3(settings, article_id, bucket_name):
    ...
    resource_path = outbox_resource_path(
        settings.storage_provider, article_id, bucket_name
    )
    storage = storage_context(settings)
        ...
        try:
            if int(version) < int(highest_version):
                ...
                message = (
                    ...
                    + " version {version} is less than highest version {highest}"
                ).format(
                    article_id=article_id, version=version, highest=highest_version
                )
                logger.info(message)
        except TypeError as exception:
            approve_status = False
            message = (
                "\nException converting version to int for {article_id}, {exc}"
            ).format(article_id=article_id, exc=str(exception))
...
def approve_by_first_vor(settings, logger, article_id, version, status, auth=True):
    ...
    first_vor = lax_provider.article_first_by_status(
        article_id, version, status, settings, auth
    )
    ...
    if not first_vor:
        ...
        and version
        and highest_version
        and int(version) < int(highest_version)
    ):
        approve_status = False
    return approve_status
...
def validate_digest(digest_content):
    ...
    if digest_content and not digest_content.summary:
        error_messages.append("Digest summary is missing")
    ...
    try:
        if digest_content and not validate_image(digest_content):
            ...
            error_messages.append("Digest image %s type is not supported" % image_file)
    except DigestValidateImageException as exception:
        error_messages.append("Validating digest image raised an exception")
