...
class TestPublishDigest(unittest.TestCase):
    ...
    def test_do_activity(
        ...
        fake_get_digest.return_value = digest_get_data(
            test_data.get("existing_digest_json"), test_data.get("stage")
        )
        fake_put_digest.return_value = test_data.get("put_response")
        activity_data = digest_activity_data(ACTIVITY_DATA, test_data.get("status"))
        fake_article_versions.return_value = (
            200,
            shared_test_data.lax_article_versions_response_data,
        )
        # do the activity
        ...
        self.assertEqual(
            self.activity.statuses.get("approve"),
            test_data.get("expected_approve_status"),
            "failed in {comment}".format(comment=test_data.get("comment")),
        )
        self.assertEqual(
            self.activity.statuses.get("put"),
