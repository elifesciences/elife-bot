...
class TestIngestDigestToEndpointDoActivity(unittest.TestCase):
    ...
    @data(
        {
            ...
            "expected_log_info": ["Digest stage value preview"],
        },
        {
            ...
            "expected_log_info": ["Digest stage value published"],
        },
    )
    def test_do_activity(
        ...
        if self.activity.digest_content and test_data.get("expected_json_contains"):
            ...
            for expected in test_data.get("expected_json_contains"):
                self.assertTrue(
                    (
                        expected in json_string,
                        "failed in json_content in {comment}".format(
            ...
            for loginfo in test_data.get("expected_log_info"):
                self.assertTrue(
                    ...
                    'failed in {comment}, "{loginfo}" not in loginfo'.format(
                        comment=test_data.get("comment"), loginfo=loginfo
                    ),
                )
    ...
    def test_do_activity_approve_false(
        self,
        test_data,
        fake_emit,
        fake_session,
    ...
    @patch("activity.activity_IngestDigestToEndpoint.get_session")
    @patch.object(activity_object, "emit_monitor_event")
    def test_do_activity_no_digest(
        self,
        fake_emit,
        fake_session,
        fake_provider_storage_context,
    ...
    @patch.object(activity_object, "emit_monitor_event")
    @patch("activity.activity_IngestDigestToEndpoint.get_session")
    def test_do_activity_docx_exists_exception(
        self,
        fake_session,
        fake_emit,
        fake_first,
    ...
    @patch.object(digest_provider, "storage_context")
    @patch("activity.activity_IngestDigestToEndpoint.get_session")
    def test_do_activity_bad_download(
        self,
        fake_session,
...
class TestIngestDigestToEndpointEmit(unittest.TestCase):
    ...
    def test_activity_end_message_ingest(self):
        ...
        expected = (
            "Finished ingest digest to endpoint for 353. Statuses {'ingest': True}"
...
class TestIngestDigestToEndpointApprove(unittest.TestCase):
    ...
    def test_approve(
        self, test_data, fake_highest_version, fake_version_map, fake_doc_exists
    ):
...
class TestDigestDownloadDocx(unittest.TestCase):
    ...
    def test_digest_download_docx_from_s3_exception(self, fake_download_docx):
        ...
        with self.assertRaises(Exception) as test_exception:
            activity_module.digest_download_docx_from_s3(
                "00666", None, None, None, None
            )
        self.assertEqual(
            str(test_exception.exception),
...
class TestDigestImageFileName(unittest.TestCase):
    ...
    def test_digest_image_file_name_from_s3_exception(self, fake_image_file_name):
        fake_image_file_name.side_effect = Exception(
...
class TestDownloadJatsForDigest(unittest.TestCase):
    ...
    def test_download_jats_for_digest_exception(self, fake_download_jats):
        fake_download_jats.side_effect = Exception(
