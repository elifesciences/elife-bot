...
test_data = {"run": "1ee54f9a-cb28-4c8e-8232-4b317cf4beda"}
...
class TestReadyToPublish(unittest.TestCase):
    def setUp(self):
        self.readytopublish = activity_ReadyToPublish(
    ...
    def test_ready_to_publish(
        self, fake_emit_monitor, fake_session, fake_set_monitor_property
    ):
    ...
    @patch("activity.activity_ReadyToPublish.get_session")
    @patch.object(activity_ReadyToPublish, "emit_monitor_event")
    def test_ready_to_publish_error(
        self, fake_emit_monitor, fake_session, fake_prepare
    ):
        fake_session.return_value = FakeSession(session_example)
    ...
    def test_preview_path(self):
        result = activity_module.preview_path(
            settings_mock.article_path_pattern, "34427", "1"
        )
