...
class tests_PreparePostEIF(unittest.TestCase):
    def setUp(self):
        ...
        activity_pass = {
            ...
            "message": data.PreparePostEIF_message,
            "expected": data.PreparePostEIF_json_output_return_example
        }
        activity_pass = {
            "update_date": None,
            "message": data.PreparePostEIF_message_no_update_date,
            "expected": data.PreparePostEIF_json_output_return_example_no_update_date
        }
    ...
    def test_activity(self, fake_session_mock, fake_s3_mock, fake_key_mock,
        ...
        for test_data in self.do_activity_passes:
            ...
            self.assertEqual(json.dumps(test_data["message"]), data_written_in_test_queue)
