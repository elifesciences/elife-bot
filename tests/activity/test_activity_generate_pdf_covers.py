...
class TestGeneratePDFCovers(unittest.TestCase):
    def setUp(self):
        ...
        self.article_snippet = {
            "subjects": [{"id": "neuroscience", "name": "Neuroscience"}]
        }
    ...
    def test_do_activity_bad_data(self):
        ...
        self.assertEqual(
            self.fake_logger.logerror[:36], "Error retrieving basic article data."
        )
    ...
    def test_do_activity_error_404(
    ...
    @patch("requests.post")
    @patch.object(activity_GeneratePDFCovers, "emit_monitor_event")
    def test_do_activity_error_500(
        self, fake_monitor_event, fake_request, fake_snippet
    ):
        ...
        self.assertEqual(
            self.fake_logger.logerror[:44],
            "unhandled status code from PDF cover service",
        )
        self.assertEqual(result, self.generatepdfcovers.ACTIVITY_PERMANENT_FAILURE)
        json.dumps(self.fake_logger.logerror)
    ...
    def test_do_activity_error_wrong_result_from_covers(
        ...
        self.assertEqual(
            self.fake_logger.logerror[:44], "Unexpected result from pdf covers API."
        )
        self.assertEqual(result, self.generatepdfcovers.ACTIVITY_PERMANENT_FAILURE)
        json.dumps(self.fake_logger.logerror)
