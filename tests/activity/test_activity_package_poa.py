...
class TestPackagePOA(unittest.TestCase):
    ...
    def fake_download_latest_csv(self):
        ...
        for file_name_path in csv_files:
            ...
            shutil.copy(
                file_name_path, os.path.join(self.poa.directories.get("CSV"), file_name)
            )
    ...
    def fake_copy_pdf_to_hw_staging_dir(self, decap_pdf):
        if decap_pdf:
            ...
            dest_doc = os.path.join(
                self.poa.directories.get("DECAPITATE_PDF"), decap_pdf
            )
            try:
                shutil.copy(source_doc, dest_doc)
            except IOError:
    ...
    @data(
        ...
        # test not a zip file
        ("tests/test_data/poa/poa_abstract.csv", None),
    )
    ...
    def test_do_activity(
        ...
        with open(bucket_list_file, "rb") as open_file:
            fake_ejp_bucket_file_list.return_value = json.loads(
    ...
    def boolean_assertion(self, value, expected, scenario=None):
        ...
        self.assertEqual(
            ...
            "{value} does not equal {expected}, scenario {scenario}".format(
                value=value, expected=expected, scenario=scenario
            ),
        )
    ...
    @patch("provider.ejp.EJP.latest_s3_file_name_by_convention")
    @patch("provider.ejp.EJP.ejp_bucket_file_list")
    def test_download_latest_csv(
        self, fake_ejp_bucket_file_list, fake_by_convention, fake_storage_context
    ):
    ...
    def test_download_latest_csv_ejp_exception(
        self, fake_ejp_s3_file_name, fake_storage_context
    ):
        ...
        self.poa.download_latest_csv()
        # make assertions
        loginfo_message = "PackagePoA unable to download CSV file for poa_author"
        self.assertTrue(loginfo_message in self.logger.loginfo)
