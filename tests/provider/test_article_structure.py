...
class TestArticleStructure(unittest.TestCase):
    ...
    @data(
        ...
        {
            "filename": "elife-00013-vor-v1-20121015000000.zip",
    ...
    def test_get_file_type_from_zip_filename(self, filename, expected):
        ...
        self.assertEqual(
            result, expected, "failed on input %s, expected %s" % (input, expected)
        )

    @unpack
    @data({"filename": "Video_22.zip", "expected": None})
    ...
    def test_inline_figure(self, filename, expected):
        self.assertEqual(
            article_structure.inline_figure(filename), expected, "Case %s" % filename
        )
    ...
    def test_get_inline_figures_for_iiif(self):
        ...
        self.assertListEqual(
            article_structure.get_inline_figures_for_iiif(files), expected
        )
