...
from provider.digest_provider import (
    ErrorCallingDigestException,
    DigestValidateImageException,
)
...
class TestDigestProvider(unittest.TestCase):
    ...
    def test_put_digest_request_headers(self, mock_requests_put):
        ...
        self.assertEqual(
            headers["Content-Type"], "application/vnd.elife.digest+json; version=1"
        )
...
class TestDigestJats(unittest.TestCase):
    def setUp(self):
        ...
        self.digest_config = digest_provider.digest_config(
            settings_mock.digest_config_section, settings_mock.digest_config_file
        )
...
class TestValidateDigest(unittest.TestCase):
    ...
    def test_validate_digest_digest_no_caption(self):
