...
class activity_CopyGlencoeStillImages(Activity):
    ...
    def end_event_glencoe_404(self, article_id, version, run):
        self.logger.info(
            "Glencoe returned 404, therefore article %s does not have videos",
            article_id,
        )
        return [
            self.settings,
    ...
    def get_glencoe_metadata(self, article_id, version, run, has_videos):
        ...
        try:
            ...
            end_event = self.end_event_checking_error(
                exception, article_id, version, run
            )
            return metadata, end_event, self.ACTIVITY_PERMANENT_FAILURE

    def handle_glencoe_metadata_assertion(
        self, exception, article_id, version, run, has_videos
    ):
        ...
        if first_chars_error == "article has no videos":
            if has_videos:
                ...
                end_event = self.end_event_glencoe_retry(
                    exception, article_id, version, run
                )
    ...
    def process_glencoe_metadata(self, metadata, article_id, version, run):
        ...
        try:
            ...
            dashboard_message = (
                "Finished Copying Glencoe still images to CDN: %sArticle: %s"
            ...
            self.logger.exception("Error when copying Glencoe still images.")
            end_event = self.end_event_copying_error(
                exception, article_id, version, run
            )
            return end_event, self.ACTIVITY_PERMANENT_FAILURE
    ...
    def store_file(self, path, article_id):
        ...
        if request.status_code == 200:
            ...
            storage.set_resource_from_string(
                resource, request.content, content_type=request.headers["content-type"]
            )
            return jpg_filename
        else:
            raise RuntimeError(
                "Glencoe returned a %s status code for %s" % (request.status_code, path)
            )
    ...
    def validate_jpgs_against_cdn(self, cdn_all_files, cdn_still_jpgs):
        ...
        self.logger.info(
            "cdn_still_jpgs_no_extension " + str(cdn_still_jpgs_no_extension)
        )
            ...
            if (
                len(
                    list(
                        [
                            fname
                            for fname in cdn_all_files_no_extension
                            if fname == still
                        ]
                    )
                )
        ...
        self.logger.info(
            "cdn_still_jpgs_without_video " + str(cdn_still_jpgs_without_video)
        )
