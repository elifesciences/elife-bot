...
class activity_SendDashboardProperties(Activity):
    def __init__(self, settings, logger, client=None, token=None, activity_task=None):
        ...
        self.description = (
            "Send selected properites of article being processed to the dashboard"
        )
        self.logger = logger
    ...
    def do_activity(self, data=None):
        ...
        self.emit_monitor_event(
            ...
            "start",
            "Starting send of article properties to dashboard for article "
            + article_id,
        )
        ...
        try:
            self.emit_monitor_event(
                ...
                "Send dashboard properties",
                "end",
                "Article properties sent to dashboard for article  " + article_id,
            )
    ...
    def set_dashboard_properties(self, soup, article_id, version):
        ...
        self.set_monitor_property(
            self.settings, article_id, "doi", doi, "text", version=version
        )

        title = utils.tidy_whitespace(parser.full_title(soup))
        self.set_monitor_property(
            self.settings, article_id, "title", title, "text", version=version
        )

        status = utils.article_status(parser.is_poa(soup))
        self.set_monitor_property(
            self.settings, article_id, "status", status, "text", version=version
        )
        ...
        self.set_monitor_property(
            self.settings, article_id, "authors", authors_text, "text", version=version
        )
