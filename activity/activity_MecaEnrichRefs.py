...
SESSION_ATTEMPT_COUNTER_NAME = "meca_enrich_refs_attempt_count"
...
class activity_MecaEnrichRefs(Activity):
    def __init__(self, settings, logger, client=None, token=None, activity_task=None):
        super(activity_MecaEnrichRefs, self).__init__(
            settings, logger, client, token, activity_task
        )

        self.name = "MecaEnrichRefs"
        self.version = "1"
        self.default_task_heartbeat_timeout = 30
        self.default_task_schedule_to_close_timeout = 60 * 5
        self.default_task_schedule_to_start_timeout = 30
        self.default_task_start_to_close_timeout = 60 * 5
        self.description = "Validate and enrich ref citaitons in MECA XML"
    ...
    def do_activity(self, data=None):
        ...
        if run_type == "silent-correction":
            self.logger.info(
                "%s, will not be invoked as part of a silent-correction, skipping"
                % self.name
            )
            return self.ACTIVITY_SUCCESS

        if not hasattr(self.settings, "meca_enrich_refs_endpoint"):
            self.logger.info(
                "%s, meca_enrich_refs_endpoint in settings is missing, skipping"
                % self.name
            )
            return self.ACTIVITY_SUCCESS
        if not self.settings.meca_enrich_refs_endpoint:
            self.logger.info(
                "%s, meca_enrich_refs_endpoint in settings is blank, skipping"
                % self.name
            )
            return self.ACTIVITY_SUCCESS
        endpoint_url = self.settings.meca_enrich_refs_endpoint
        ...
        self.logger.info("%s, updating enriched XML to %s" % (self.name, s3_resource))
