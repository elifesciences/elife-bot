...
class activity_ExpandArticle(Activity):
    def __init__(self, settings, logger, client=None, token=None, activity_task=None):
        ...
        self.description = (
            "Expands an article ZIP to an expanded folder, renaming as required"
        )
        self.logger = logger
    ...
    def do_activity(self, data=None):
        ...
        if status is None or (status != "vor" and status != "poa"):
            self.logger.error(
                "Name '%s' did not match expected pattern for status"
                % filename_last_element
            )
            return (
                self.ACTIVITY_PERMANENT_FAILURE
            )  # status could not be determined, exit workflow.
        ...
        try:
            ...
            self.emit_monitor_event(
                ...
                "Expand Article",
                "error",
                "Error expanding article " + article_id + " message:" + str(exception),
            )
    ...
    def check_filenames(self, filenames):
        ...
        if not xml_found:
            raise RuntimeError(
                "No .xml file found. List of files found is %s" % filenames
            )
