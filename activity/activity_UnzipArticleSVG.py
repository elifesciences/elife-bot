...
"""
UnzipArticleSVG activity
"""

class activity_UnzipArticleSVG(activity.activity):
  
  def __init__(self, settings, logger, conn = None, token = None, activity_task = None):
    activity.activity.__init__(self, settings, logger, conn, token, activity_task)

    self.name = "UnzipArticleSVG"
    self.version = "1"
  ...
  def do_activity(self, data = None):
    ...
    for tmp_doc_path in tmp_document_path_list:
      ...
      s3key_name = self.get_svg_object_S3key_name(elife_id, tmp_document)
    ...
    if(self.logger):
      self.logger.info('UnzipArticleSVG: %s' % elife_id)
  ...
  def get_svg_object_S3key_name(self, elife_id, document):
    """
